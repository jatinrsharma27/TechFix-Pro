...
export default function AdminNavbar({ onToggleSidebar, sidebarOpen }: AdminNavbarProps) {
  ...
  useEffect(() => {
    ...
    console.error = (...args) => {
      if (args[0]?.includes?.('Extension') || args[0]?.includes?.('runtime.lastError')) {
        return;
      }
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await fetch(`/api/notifications?userType=admin&userId=${adminId}`);
      ...
      // Silently handle notification errors to prevent breaking the UI
      setNotifications([]);
      setUnreadCount(0);
    }
  };
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/search?q=${encodeURIComponent(query)}&limit=12&type=all`);
      if (response.ok) {
        const data = await response.json();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className={`flex items-center space-x-2 ${showMobileSearch ? 'hidden' : 'block'}`}>
            ...
            <h1 className="text-lg sm:text-xl font-semibold text-gray-900 hidden sm:block">TechFix Pro Admin</h1>
            <h1 className="text-lg font-semibold text-gray-900 sm:hidden">Admin</h1>
          </div>
        </div>

        {/* Middle Section - Search Bar */}
        <div className={`flex-1 max-w-2xl mx-4 sm:mx-12 ${showMobileSearch ? 'block' : 'hidden md:block'}`} ref={searchRef}>
          <form onSubmit={handleSearch} className="relative">
            <input
              type="text"
              placeholder="Search requests, users, employees, revenue..."
              value={searchQuery}
