  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await fetch('/api/admin/auth', {
        ...
        body: JSON.stringify({ action: 'verify-otp', email, otp: otpCode, purpose })
      });
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          if (purpose === 'reset') {
            router.push(`/admin/Auth/set-password?email=${encodeURIComponent(email)}`);
          } else if (purpose === 'signup') {
            // For signup, redirect to set password page
            router.push(`/admin/Auth/set-password?email=${encodeURIComponent(email)}`);
          } else if (purpose === 'signin' && data.user) {
            // For signin, set user data and redirect to dashboard
            localStorage.setItem('admin', JSON.stringify(data.user));
            localStorage.setItem('admin_session_token', `admin_${data.user.id}_${Date.now()}`);
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            We have sent a 6-digit verification code to your admin email
          </p>
